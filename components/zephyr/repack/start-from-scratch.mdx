...
Open the `rspack.config.js` file in `HostApp` and modify the `build` configuration to include the following: 


```js title="HostApp/rspack.config.js"{4,15,68}
import path from 'node:path';
import {fileURLToPath} from 'node:url';
import * as Repack from '@callstack/repack';
...
```js title="MiniApp/rspack.config.js"{4,19,68}
import path from 'node:path';
import {fileURLToPath} from 'node:url';
import * as Repack from '@callstack/repack';
import {withZephyr} from 'zephyr-repack-plugin';
...
<Image src="/host-app-ios.jpg" alt="Zephyr Cloud Minimal example running in iOS emulator" />
