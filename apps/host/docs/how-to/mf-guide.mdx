...
At the moment. when we are resolving your remote dependencies, we will need the [remotes](/reference#remote-application) to be built first. Be wary that the first built remotes should not be consuming any other remotes. 
