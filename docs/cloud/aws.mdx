...
# --- 1) CloudFront: detach function associations and disable distribution ---
if [[ -n "$CF_DIST_ID" && "$CF_DIST_ID" != "DISTRIBUTION_ID" ]]; then
  log "Detaching functions and disabling CloudFront Distribution: $CF_DIST_ID"
  ...
  if [[ -n "$DIST_CONF_JSON" ]]; then
    ...
    log "Waiting for Distribution status to become Deployed (after disable)"
    for i in {1..90}; do
...
delete_lambda_version_with_wait() {
  local version_arn="$1"
  local max_tries="${2:-120}"   # up to ~20–30 min в сумме при 10–15 сек интервале
  local sleep_s="${3:-15}"

  log " delete version: $version_arn"

  for ((i=1; i<=max_tries; i++)); do
    # 1) Убедимся, что CloudFront больше не ссылается на эту версию
    local refs
    ...
    # 2) Попытаемся удалить. Если вернёт replicated error — подождём и повторим.
