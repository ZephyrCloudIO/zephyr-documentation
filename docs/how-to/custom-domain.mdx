...
To do so, they can assign a custom domain to a specific version or [latest version under a tag](/how-to/versioning-tags) via our **environment settings** under each applications. The latest deployment's version URL of a tag will always be updated when you deploy an application through terminal when the tag's criteria is satisfied. 

[Read more about tags, versions and tag rules](/how-to/versioning-tags#tag-rules-on-dashboard). 
...
If you haven't done it yet, you can start to deploy your first application on Zephyr via our [Get Started Guide](/general/get-started) and configure your custom **cloud provider** as [Cloudflare](/cloud/cloudflare) and [Netlify](/cloud/netlify). 
...
b. Click on **Create Tag** and insert required input field **Tag name** and select one of the [**Tag conditions**](/how-to/versioning-tags#tag-rules-on-dashboard). 
...
c. Run a build command to deploy your application and verify that the latest deployed version is attached to the tag you just created. 
    - You can find out by either by clicking on the **Version** next to that tag name on Zephyr's dashboard, or select from dropdown on [Chrome Extension](https://chromewebstore.google.com/detail/zephyr-mission-control/liflhldchhinbaeplljlplhnbkdidedn) to inspect whether your changes are being deployed. 
...
- [Configure Custom Domain On Cloudflare](#cloudflare)

- [Configure Custom Domain on Netlify](#netlify)
