...
const config = {
  ...
  watchFolders: [
    // Add local workspace/package paths only if Metro must watch files
    // outside this app. Example:
...
const getConfig = async () => {
  const zephyrConfig = await withZephyr()({
    name: 'hostApp',
    remotes: {
      // Match the port used by your MiniApp Metro server in local development.
