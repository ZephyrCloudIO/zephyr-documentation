...
Zephyr provides hooks to tap into the deployment lifecycle. Currently available:

#### onDeployComplete

Fires after a successful deployment with metadata about the deployed application. Useful for integrating with external services like registries or notification systems.
...
- `snapshot` - Build snapshot details (app name, build ID, git info)
- `federatedDependencies` - Array of federated dependencies
- `buildStats` - Build statistics and context
