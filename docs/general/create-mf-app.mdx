...
For example we have [react-tractor-sample](https://github.com/ZephyrCloudIO/zephyr-examples/tree/main/examples/react-tractor-sample).
There is app `team-red` that consumes another two app `team-green` and `team-blue`.
To use specific version of each app define `zephyrDependencies` in `package.json` like this.
...
```
If omit this configuration will be uses latest version of remote.
