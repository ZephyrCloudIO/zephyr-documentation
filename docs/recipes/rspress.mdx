...
\*\*The total size for the (web) thread represents the original files but the uploaded total size means the SSG files already processed. If the SSG flag is disabled the Rspress plugin relies in the Rspack plugin execution.
