...
Because of how Vite and Rollup exposing hooks and processing modules in plugins, the sequence of plugin matters where `withZephyr()` should be after `react()` but before any other plugins. For example, if `svgr()` is before `withZephyr()` it would interrupt transformed output bundle and not as expected. 
