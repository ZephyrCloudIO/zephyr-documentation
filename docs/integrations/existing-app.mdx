...
<Tabs values={[{ label: 'Rspack' }, { label: 'webpack' }]}>
  <Tab>
    ```bash filename="Terminal" npm i zephyr-rspack-plugin@latest --save ```
  </Tab>
  <Tab>
    ```bash filename="Terminal" npm i zephyr-webpack-plugin@latest --save ```
  </Tab>
</Tabs>
...
Read more about Vite in [React + Vite](/integrations/react-vite) and [Vite + Rspack + Webpack with Module Federation](/integrations/vite-rspack-webpack-mf).
