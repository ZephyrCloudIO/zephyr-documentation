...
Install the `zephyr-edge-contract` and `zephyr-webpack-plugin` by copying them from the Zephyr examples repository.

:::warning
Currently, these packages are only available in the Zephyr Examples repository during the Alpha 1 testing phase and are expected to be accessible via NPM in the future. For more details, visit the [Zephyr Examples repository](https://github.com/ZephyrCloudIO/zephyr-examples/tree/main/_zephyr_pack).
:::

To add these packages to your project, include them as dependencies in your `package.json`. Note that the file path used below is specific to our examples repository, which distributes the packages across multiple examples.
...
:::info
The build process generates a series of URLs for each deployment. For a comprehensive understanding of these build outputs, please consult our [versioning documentation](/guide/concepts/versioning).
