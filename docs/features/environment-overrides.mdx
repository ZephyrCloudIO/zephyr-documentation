...
{
  "zephyr:dependencies": {
    "header": "ui-components@stable",
    "cart": "cart-service@^2.0.0"
  }
}
```
...
Frontend and backend teams work independently with different deployment cadences:

```
Frontend staging: points to backend@development
Frontend production: points to backend@production

Backend team deploys at their own pace
Frontend automatically gets appropriate backend version per environment
```
...
This means environment overrides always take precedence over the build-time configuration, giving you full control at deployment time.
