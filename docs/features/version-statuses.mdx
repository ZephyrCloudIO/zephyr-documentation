...
description: Track and manage application versions with statuses including Live, Available, Deprecated, Unavailable, Failed, and Rolled back.
head:
  - - meta
    - property: og:description
      content: Track and manage application versions with statuses including Live, Available, Deprecated, Unavailable, Failed, and Rolled back.
---
...
- **[Versions](/features/versions)** - Complete guide to version management and lifecycle
- **[Tags & Environments](/features/tags-environments)** - Deploy versions to specific environments
- **[Instant Rollbacks](/features/instant-rollbacks)** - Roll back to previous versions instantly
...
![alt 'Version statuses column'](/version-statuses-column.webp)

### Updating Version Status

To update a version’s status, click the Actions button next to a version in the list.
![alt 'Version history actions'](/version-history-actions-btn.webp)
This will open a dropdown menu with the following options:

- **Deploy Version** \
  You can deploy version with any status except unavailable. If you attempt to deploy a Deprecated version, you will see a warning message. You must acknowledge the risks by checking a confirmation box before proceeding. Learn more about [deploying to environments](/features/tags-environments#environments).
  ![alt 'Deploy modal'](/deploy-modal-with-warning.webp)
