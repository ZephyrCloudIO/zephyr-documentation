...
### Environment Variables (`ZE_PUBLIC_*`)
...
- ✅ **Safe for `ZE_PUBLIC_*`**: API endpoints, feature flags, CDN URLs, public analytics IDs, client-side configuration
- ❌ **Never use `ZE_PUBLIC_*` for**: API secrets, database passwords, authentication tokens, private keys, sensitive credentials
