...
head:
  - - meta
    - property: og:description
      content: Quickly revert to previous application versions with zero downtime
---

# Instant Rollbacks

Instant rollbacks let you immediately revert to a previous [version](/features/versions) of your application when something goes wrong. Whether you discover a bug in production or need to quickly undo a problematic deployment, rollbacks get your application back to a working state in seconds.
...
**Environment rollbacks**: Roll back a specific [environment](/features/tags-environments#environments) like production or staging. This affects only that environment while leaving others unchanged.

**Tag rollbacks**: Roll back a [tag](/features/tags-environments#tags) like "latest" or "stable". This may affect multiple environments that use that tag, depending on your configuration.
...
**Feature flags**: Use [environment variables](/features/environment-overrides#environment-variable-overrides) or feature flags to control new functionality, allowing you to disable features without full rollbacks.
...
Zephyr's [remote dependency system](/features/remote-dependencies) helps manage this by allowing you to specify version constraints and compatibility rules.
