...
```bash
npx @react-native-community/cli@latest init HostApp
cd HostApp
```
...
```bash
npx @react-native-community/cli@latest init MiniApp
cd MiniApp
```
...
This configures Zephyr remote resolution and the local manifest, but it does not upload a bundle. See [Metro configuration and publication](/bundlers/metro#configuration-and-publication) for the lifecycle, Git-based identity inference, and an explicit `zephyr.config.ts` identity override.
...
### Step 2: Register Publication Commands

Create `react-native.config.js` in both applications. The adapter registers the remote and host publication commands:
...
If either application already has React Native CLI settings, merge the adapter commands instead of replacing the existing config. See [React Native CLI publication commands](/bundlers/metro#react-native-cli-publication-commands) for the preserving form.

:::details{title="Why This Configuration?"}

This configuration adds React Native CLI commands that:

- Handle Module Federation bundling
- Integrate with Zephyr Cloud for asset upload
- Update the Module Federation manifest
- Enable deployment to Zephyr's edge network
...
After the first bundle finishes, open the Zephyr Cloud dashboard and verify that the mini application exists with a new version. Then configure its environments and tags in [Tags & Environments](/features/tags-environments).
...
- **remotes**: Mini-applications to load
  ...
  - Value is a local manifest URL for development or a Zephyr selector such as `zephyr:miniApp@yourEnvironment` for cloud resolution
...
- **runtimePlugins**: Optional Module Federation runtime plugins for custom logic
...
### Step 3: Publish the Host Application

Run the host publication command when you need to upload a host version. Keep the bundle output under `dist/<platform>` so Zephyr can discover the artifacts:

```bash
npx react-native bundle-mf-host --platform ios --dev false --entry-file index.js --bundle-output dist/ios/host.bundle
```

Verify the first host version in the dashboard just as you did for the mini application. For RNEF projects, use the required plugin and commands in the [Metro bundler guide](/bundlers/metro#rnef-publication-commands).
...
During development, the host will load mini-applications from the URLs specified in your Metro configuration. In production, Zephyr will automatically resolve these to your deployed versions based on your environment configuration. `start`, `run-ios`, and `run-android` do not publish an application version; run `bundle-mf-remote` or `bundle-mf-host` explicitly to build and upload one.
...
- Add [React Native OTA Updates](/features/react-native-ota-updates) with `zephyr-native-cache`
- Review the [OTA example repository](https://github.com/ZephyrCloudIO/zephyr-native-cache-test) for a full host/remotes setup
