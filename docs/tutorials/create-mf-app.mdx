...
We conducted tests using React, but other frameworks may also be compatible. See a list of supported framework [here](/tutorials/mf-guide).
...
<Tabs values={[{ label: 'Rspack' }, { label: 'Webpack' }]}>
  <Tab>
    ```bash cd your-app-name npm install zephyr-rspack-plugin --save-dev ```
  </Tab>
  <Tab>
    ```bash cd your-app-name npm install zephyr-webpack-plugin --save-dev ```
  </Tab>
</Tabs>
